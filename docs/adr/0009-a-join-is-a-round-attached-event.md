...
Play's subtitle still counts an orphaned joiner in its "3 players", because it reads the roster rather than the Standings. Home's card says the same thing about the same Match, and — worth knowing before anyone reaches for a one-line fix — says it in its own code rather than through a shared accessor: `PlayView.playSubtitle` and `HomeCard` build the phrase independently. So there is no one place to change, and changing one alone would have two screens disagree about how many people are at a table. Known, and left; the duplication predates this decision and is the thing to fix first if the count is ever made to follow the Standings.
