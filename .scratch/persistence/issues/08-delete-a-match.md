...
**Two bugs found in use, and fixed** (this branch). Both of the notes left open above turned out to be real, and both were reproduced on the simulator by driving the app rather than by reasoning about it.

- **Deleting a Match crashed the app when another Match was left on Home.** The crash report is unambiguous: `MatchCard.body` → `entrantsText` → `Match.mode.getter` → a SwiftData assertion. Home handed each row the Match itself, and SwiftUI redraws a row in the same turn the Match behind it is deleted — every property of a deleted model traps rather than answering. It only showed up with a second Match on screen to redraw alongside the deleted one, which is why deleting the last Active Match looked fine and deleting one of two Archived ones did not. Home now draws its list from `HomeCard`, a value read off the Match when Home's body runs — the same reasoning `PendingDeletion` was already built on. `Match.isGone` covers the `@Query` array naming a Match that has gone; note that `isDeleted` alone does not, since SwiftData reports it only until the deletion is saved.
- **The confirmation looked broken.** `DecisionSheet` asked for a `.medium` detent and a clear background, so the surface was the bottom of a half-screen sheet and the rest was the system's translucency: a blurred, frozen copy of Home above the question. The sheet is now sized to the surface it draws — measured, because the wording decides the height — and backed by the shell's own colour, so it reads as one card. Presenting from a `.contextMenu` action was investigated and cleared: delaying the presentation until the menu had gone changed nothing.
