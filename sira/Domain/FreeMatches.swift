...
/// the meter draws.
///
/// `started` is deliberately not clamped on the way in. It is the tally as
/// stored, and a tally past the allowance is an ordinary state for a player
...
struct FreeMatches: Equatable {
    ...
    var used: Int { min(max(started, 0), Self.allowance) }
